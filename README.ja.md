...
<p align="center">
  <strong>インテリジェントモデルルーティングシステム</strong> · マルチプラットフォーム AI API の統一管理と転送
</p>
...
ModelRouter-App は、複数の AI サービス（OpenAI、アリババクラウド、智譜、DeepSeek など）を統合し、ルーティングと API キー管理によりインテリジェントな配信とコスト管理を実現するオープンソースの AI モデルルーティング管理プラットフォームです。
...
| 機能 | 説明 |
|------|------|
| スマートルーティング | プライマリモデル失敗時にバックアップモデルへ自動フェイルオーバー |
